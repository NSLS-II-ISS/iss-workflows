...
for letter in ['a', 'b', 'c', 'd', 'e']:
    _ghs_selected_gas_dict[f'ghs_selected_gas_{letter}'] = {'kind': 'epics_pv',
...
for i in range(1, 17):
    _ghs_mfc_dict[f'ghs_mfc_{i}_sp'] = {'kind': 'epics_pv',
                                        'pv_str': 'XF:08IDB-UT{Gas:1-MFC:' + f'{i:02d}' + '}F:Target-SP',
                                        'human_readable_key': f'SampleGasHandlingSystem.MFC{i}.setpoint'}
    _ghs_mfc_dict[f'ghs_mfc_{i}_rb'] = {'kind': 'epics_pv', 'pv_str': 'XF:08IDB-UT{Gas:1-MFC:' + f'{i:02d}' + '}F-I',
...
metadata_key_dict = {**metadata_key_dict, **_ghs_selected_gas_dict, **_ghs_mfc_dict}
...
metadata_key_match = {k: v['human_readable_key'] for k, v in metadata_key_dict.items()}
...
def create_file_header_from_md(md):
    output = ''
